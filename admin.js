(function () {
  ...
  function refreshCounts() {
    ...
    const withBday = Store.getMembers().filter(m => m.birthday).length;
    const bdayCount = document.getElementById('birthdays-count');
    if (bdayCount) bdayCount.textContent = withBday;
  }
    ...
    membersListEl.innerHTML = items.map(m => {
      ...
      if (m.birthday) {
        ...
        if (!isNaN(bd)) {
          ...
          const month = MONTHS_SHORT[bd.getMonth()];
          const days = daysUntilBirthday(m.birthday);
          bdayLine = `<div class="card-bday">🎂 ${day} ${month} · ${formatDaysLabel(days)}</div>`;
        }
      }
      return `
  ...
  function openMemberForm(id) {
    const member = id ? Store.getMembers().find(m => m.id === id) : { name: '', role: '', age: '', birthday: '', photo: '' };
    if (!member) return;
    showModal(id ? 'Изменить участницу' : 'Добавить участницу', `
      ...
      <label>Дата рождения
        <input name="birthday" type="date" value="${esc(member.birthday || '')}">
      </label>
  ...
  function ageOnNextBirthday(birthday) {
    if (!birthday) return null;
    const bd = new Date(birthday);
    if (isNaN(bd)) return null;
    const today = new Date();
    const nextYear = (new Date(today.getFullYear(), bd.getMonth(), bd.getDate()) < today)
      ? today.getFullYear() + 1
      : today.getFullYear();
    return nextYear - bd.getFullYear();
  }
  ...
  function formatDaysLabel(days) {
    ...
    if (days < 30) {
      ...
      return `через ${days} дн. (~${weeks} нед.)`;
    }
    const months = Math.floor(days / 30);
    return `через ${days} дн. (~${months} мес.)`;
  }
  ...
  const bdaySoonEl = document.getElementById('bday-soon');
  const bdayCalendarEl = document.getElementById('bday-calendar');
  const bdayEmptyEl = document.getElementById('bday-empty');

  function renderBirthdays() {
    if (!bdaySoonEl) return;
    const members = Store.getMembers()
      .filter(m => m.birthday)
      .map(m => ({
        ...m,
        daysLeft: daysUntilBirthday(m.birthday),
        nextAge: ageOnNextBirthday(m.birthday),
        bdDate: new Date(m.birthday),
      }))
      .filter(m => m.daysLeft !== null);

    if (bdayEmptyEl) bdayEmptyEl.style.display = members.length ? 'none' : 'block';
    document.querySelector('.bday-section-title').style.display = members.length ? '' : 'none';

    // СЛЕДУЮЩИЕ — топ-6 по близости
    const soon = members.slice().sort((a, b) => a.daysLeft - b.daysLeft).slice(0, 6);

    bdaySoonEl.innerHTML = soon.map((m, i) => {
      const day = m.bdDate.getDate();
      const month = MONTHS_RU[m.bdDate.getMonth()];
      const urgent = m.daysLeft <= 7;
      const today = m.daysLeft === 0;
      return `
        <div class="bday-card ${today ? 'today' : urgent ? 'soon' : ''}">
          <div class="bday-avatar">
            ${m.photo
              ? `<img src="${esc(m.photo)}" alt="${esc(m.name)}">`
              : `<span>${esc((m.name || '?')[0])}</span>`}
          </div>
          <div class="bday-info">
            <div class="bday-name">${esc(m.name)}</div>
            <div class="bday-role">${esc(m.role || '')}</div>
            <div class="bday-date">${day} ${month}${m.nextAge ? ` · исполнится ${m.nextAge}` : ''}</div>
            <div class="bday-counter">${formatDaysLabel(m.daysLeft)}</div>
          </div>
        </div>
      `;
    }).join('');
    ...
    members.forEach(m => byMonth[m.bdDate.getMonth()].push(m));
    byMonth.forEach(arr => arr.sort((a, b) => a.bdDate.getDate() - b.bdDate.getDate()));

    const currentMonth = new Date().getMonth();

    bdayCalendarEl.innerHTML = byMonth.map((arr, idx) => {
      if (!arr.length) return '';
      const isCurrent = idx === currentMonth;
      return `
        <div class="bday-month ${isCurrent ? 'current-month' : ''}">
          <div class="bday-month-title">${MONTHS_RU[idx]}</div>
          <div class="bday-month-list">
            ${arr.map(m => {
              const day = m.bdDate.getDate();
              const isToday = m.daysLeft === 0;
              return `
                <div class="bday-row ${isToday ? 'is-today' : ''}">
                  <span class="bday-day">${day}</span>
                  <span class="bday-mini-avatar">${m.photo
                    ? `<img src="${esc(m.photo)}" alt="">`
                    : `<span>${esc((m.name || '?')[0])}</span>`}</span>
                  <span class="bday-row-name">${esc(m.name)}</span>
                  ${m.nextAge ? `<span class="bday-row-age">${m.nextAge}</span>` : ''}
                </div>
              `;
            }).join('')}
          </div>
  ...
  Store.onMembersChange(() => { renderMembers(); renderBirthdays(); refreshCounts(); });
